...
Soft-delete and multi-tenant filtering — implemented by `RepositoryBase.ApplyDataFilters<TQueryable, TOtherEntity>` (covered on the [Repositories page](/ddd/domain-repositories)) — is *not* a manager's job either; the repository applies those `WhereIf` clauses automatically when the entity implements `ISoftDelete` or `IMultiTenant`.
