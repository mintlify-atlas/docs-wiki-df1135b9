...
The fifth (`true`) argument to `RegisterService` is `isReadOnlyRepository` — for the read-only services the registrar marks the registration so the resolved instance has `IsChangeTrackingEnabled = false`.
