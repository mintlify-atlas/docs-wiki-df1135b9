---
title: "ISpecification, Combinators and Expression Rebinding Tricks"
sidebarTitle: "Specifications"
description: "ABP's implementation of the specification pattern: ISpecification<T>, expression composition and the combinator primitives in Volo.Abp.Specifications."
---
...
`ISpecificationParser.cs` declares an optional contract for code that needs to translate an `ISpecification<T>` into a backend-specific criteria object — for example, an NHibernate `ICriteria` instance or a Mongo `FilterDefinition`:
...
The package itself does not ship a default parser — `TCriteria` is left to the consuming module to define, so a downstream Mongo or NHibernate integration can plug its own native criteria type into the same pipeline without forcing every caller to depend on it.
