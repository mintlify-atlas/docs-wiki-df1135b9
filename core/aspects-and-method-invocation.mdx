...
<Note>
  The list is `List<string>` — not a `HashSet<string>`. That's a deliberate choice: a concern may be applied multiple times legitimately (nested scopes that re-enter the same code path) and need to be removed exactly the same number of times. `RemoveAll(c => c == concern)` only nukes one application at a time via `RemoveApplied` because each call removes every match — so interceptors must be careful to use `Applying` (scoped) rather than `Add`/`Remove` pairs.
</Note>
