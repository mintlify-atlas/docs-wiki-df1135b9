...
`ClassInterceptorsSelectorList : List<NamedTypeSelector>` (implementing `IClassInterceptorsSelectorList`) is what backs `ServiceRegistrationActionList.DisabledClassInterceptorsSelectors` — see [Dependency injection](/core/dependency-injection).
