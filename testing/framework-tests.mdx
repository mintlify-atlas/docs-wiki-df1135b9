...
These projects share the embedded `Volo.Abp.TestApp` domain (Person, Phone, City, Product, Author, Book, Blog, …) and exercise it through each persistence stack. The reusable helper is `TestAppTestBase<TStartupModule> : AbpIntegratedTest<TStartupModule>` at `framework/test/Volo.Abp.TestApp/Volo/Abp/TestApp/Testing/TestAppTestBase.cs`, which adds:
...
- `framework/test/Volo.Abp.TestApp/` — the embedded domain (`Person`, `Phone`, `City`, `Product`, `Author`, `Book`, `Blog`, …), `TestAppModule : AbpModule`, `TestAppTestBase<T>`, and the `TestMemoryDistributedCache`. EF Core, MongoDB, MemoryDb, and Auditing all rely on it.
