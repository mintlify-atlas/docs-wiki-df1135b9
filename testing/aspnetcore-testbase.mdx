...
The package also reads `appsettings.secrets.json` via the `AddAppSettingsSecretsJson()` extension from `Volo.Abp.AspNetCore`, used inside both `WebApplicationFactory.CreateHost` and the legacy `CreateHostBuilder`.
