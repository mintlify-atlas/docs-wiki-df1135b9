...
| `framework/test/Volo.Abp.TestApp/` | Embedded DDD sample (`Person`, `City`, `Phone`, `District`, `TestAppModule`, `TestAppTestBase<T>`, `TestMemoryDistributedCache`) used by EF Core, Mongo, MemoryDb, Auditing. | Project reference, never published. |
