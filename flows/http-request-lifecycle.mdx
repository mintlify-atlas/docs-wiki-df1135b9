---
title: "HTTP Request Lifecycle: Middlewares, Filters, and Controllers"
...
sequenceDiagram
    ...
    Mvc->>Filters: AbpValidationActionFilter
    ...
    Filters->>Filters: AbpGlobalFeatureActionFilter
...
`AbpCorrelationIdMiddleware` (under `Volo.Abp.AspNetCore/Volo/Abp/AspNetCore/Tracing/`) reads/issues the configured correlation header and stamps `ICorrelationIdProvider`. `AbpRequestLocalizationMiddleware` (under `Volo.Abp.AspNetCore/Volo/Abp/AspNetCore/Localization/`) is a thin override of ASP.NET's `RequestLocalizationMiddleware` that respects ABP `LanguageInfo` settings.
...
The filter ordering is set by `AbpAspNetCoreMvcModule.ConfigureServices`, which calls `MvcOptions.AddAbpFilters()`:
