...
Ten projects total — four UI shapes (`Mvc`, `Host` headless, `Blazor.Server`, `Blazor.WebAssembly`) each in EF Core and MongoDB flavours, plus the `Blazor.WebAssembly/Client` and `Blazor.WebAssembly/Shared` companion libraries. The CLI step `AppNoLayersTemplateBase.GetCustomSteps` keeps only the projects matching the chosen `-u` / `-d` combination.
