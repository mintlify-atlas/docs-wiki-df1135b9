...
| `ResolveAsync("AbpIdentity")` | `Server=acme-db;Database=Acme;...` | Tenant has no `AbpIdentity` entry. No mapped database with `IsUsedByTenants` matches either, so the resolver falls through to the tenant's non-empty `Default` and returns `acme-db` *before* it would consult `base.ResolveAsync` |
