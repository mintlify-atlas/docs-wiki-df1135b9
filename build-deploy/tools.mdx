---
title: "Repository Tools — changelog, localization, nuget.exe, cleanup"
sidebarTitle: "Tools"
...
| `delete-bin-obj.ps1`                                    | PowerShell            | Anyone     | Wipe `bin/` and `obj/` everywhere                  |
| `smtp-prober-email-sender.exe`                          | .NET console          | Anyone     | Smoke-test email module without spinning a real SMTP server |
...
## smtp-prober-email-sender.exe

`smtp-prober-email-sender.exe` is a small standalone .NET console that ABP keeps at the repo root to verify the `Volo.Abp.MailKit` integration without spinning a real SMTP server. The maintainers built it as a smoke probe — point it at an `appsettings.json` containing SMTP host/port/credentials and it sends one test message:
