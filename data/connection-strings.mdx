...
The tenant directory entity owns its own `ConnectionStrings` collection; the resolver reads it via `ITenantStore` and falls back to the host config. The exact implementation is documented in [Multi‑tenancy connection strings](/multi-tenancy/connection-strings).
