...
ABP's own [testing infrastructure](/testing/testbase) registers a similar deterministic clock so audited timestamps are reproducible.
...
- [/infrastructure/auditing](/infrastructure/auditing) — `CreationTime`, `LastModificationTime` flow through `IClock`.
- [/core/options-and-configuration](/core/options-and-configuration) — where `AbpClockOptions` is registered.
- [/multi-tenancy/tenant-configuration-store](/multi-tenancy/tenant-configuration-store) — per-tenant timezone settings can drive `ICurrentTimezoneProvider`.
